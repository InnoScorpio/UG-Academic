...
{	////HW5 for Friday 2/16/2018         Do not touch any other parts of this program (stay inside callsam())
	...
	/**/	R  0.1, 0., 1., 0.);		glutWireCube(20.);		//I activated this line for you to start, it remain 7 more lines for your experiment
    	M   R	10., 0.,0.,1);		glutWireCube(2.);	m
    //		R	spin,0.,0.,1);		glutWireCube(2.);
    //	M   R	spin,0.,0.,1);		glutWireCube(2.);	m

    //	T	      .0001, 0., 0.);	glutWireCube(10.);
	//	M   T	        3.,0.,0.);	glutWireCube(2.);	m
	//		T	spin*.0001,0.,0.);	glutWireCube(20.);
	...
	//==Other code ==
		...
		//	M   T	        3.,0.,0.);	glutWireCube(2.);	m
		//	T	spin*.0001,0.,0.);	glutWireCube(2.); m
		//	M   T	spin*.1   ,0.,0.);	glutWireCube(2.);	m
		//	M   R  spin*1.5 ,0.,0., 0.);   glutWireSphere(1., 30, 20); m
...
//float an = 0., da = 10.;
int n = 10; float an = 0., da = 1. / n; //GLfloat xRotated, yRotated, zRotated;
void bar() {
	M S 1., 10., 1.); glutSolidCube(1.);  m
}
void halfRobot(float w) //w width between the two areas
{
	an += da; if (an <= -360. || an >= 360.) da = -da;
	M T - w, 0., .0); R - an*.1, 1., 0., 0.); T - w, -5., 0.); L1 bar(); l1 m; //left member
	M  T  w, 0., .0); R  an*.1, 1., 0., 0.); T  w, -5., 0.); L2 bar();  l2 m;	//right member
																				//First 
																				//an+=da;
																				//if (an <= -360. || an >= 360.) da = -da;
																				//M T -1.,0.,0.); R -an*.1,1.,0.,0.); T -1.,-5.,0.); L1 bar(); L1 m;
																				//M T  1.,0.,0.); R  an*.1,1.,0.,0.); T  1.,-5.,0.); L2 bar(); L2 m;
}
...
void terrain()
{
	...
	//glEnd();
	//C 1.0, 0.0, 0.0);  M  T 0., -15., 0.); S 60., .01, 60.); L1; Scub 10.); l1  m
	//glColor3f(1.0, 0.0, 0.0);
...
void Robot()
{
	M S 3., 1., 1.6); bar();  m M T 0., 8., 0.); Ssph 3., 8, 4); m; //body, head
...
void DancingRobots()      //call this function from the display function
{
	R .2, 0., 1., 0.);
	M T - 40., 0., 0.); for (int i = 0; i < n; i++) { T 50., 0., 0.); R 16.*i, 0., 1., 0.); Robot(); } m
		M  T 0., -15., 0.); S 60., .01, 60.); L1; Scub 10.); l1  m
		//M  T 0., -15., 0.); S 60., .01, 60.); L1; Scub 10.); l1  m		//ground
}
...
void init(void)
{ //initParticles();
  //PlaySound("LASER.wav", NULL, SND_FILENAME); //or SND_LOOP
  //Useless code PlaySound((LPCWSTR)"tone.wav", 0, SND_LOOP | SND_ASYNC|[B]SND_FILENAME[/B]);
  //	PlaySound(TEXT("C:\\Users\\Jose Dixon\\Desktop\\Robot_blip-Marianne_Gagnon-120342607.wav"), NULL, SND_LOOP| SND_FILENAME);
...
void main(int argc, char **argv)
{
	...
	glutCreateWindow("Dr. Tannouri's walking robot animation");
	init();
