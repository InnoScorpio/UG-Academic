breed [ balls piece ] ;; pieces fall through the air...

balls-own
[ xx yy xv yv
]

;; these are the piece's offsets relative to turtle 0
]
...
to-report move-left-clear?
  report all? pieces [clear? patch-at y (- x)]
end

to-report move-right-clear?
  report all? pieces [clear? patch-at y (- x)]
end

to-report move-up-clear?
  report all? pieces [clear? patch-at y (- x)]
end

to-report move-down-clear?
  report all? pieces [clear? patch-at y (- x)]
end

to-report move-worthwest-clear?
  report all? pieces [clear? patch-at y (- x)]
end

to-report move-wortheast-clear?
  report all? pieces [clear? patch-at y (- x)]
end

to-report move-southwest-clear?
  report all? pieces [clear? patch-at y (- x)]
end

to-report move-southeast-clear?
  report all? pieces [clear? patch-at y (- x)]
end

to-report circle3-clear?
  report all? pieces [clear? patch-at y (- x)]
end
...
to setup-piece  ;; Piece Procedure
 setup-t  set color green
    setxy
      ([xcor] of turtle 0) + x
      ([ycor] of turtle 0) + y
end
...
to setup-piece5Z
  setup-5Z set color pink
 setxy
 ([xcor] of turtle 0) + x
 ([ycor] of turtle 0) + y
end

to setup-piece5T  ;;T-shaped
  setup-5T set color magenta
  setxy
   ([xcor] of turtle 0) + x
   ([ycor] of turtle 0) + y
end

to setup-pieceSQ  ;; 4 Box
  setup-SQ set color orange
  setxy
  ([xcor] of turtle 0) + x
  ([ycor] of turtle 0) + y
end
...
to setup-5C ; Cardinal 5
   if (who = 1) [ set x -1 set y  0 ]
   if (who = 2) [ set x  1 set y  0 ]
   if (who = 3) [ set x  0 set y -1 ]
   if (who = 4) [ set x  0 set y  1 ]
   if (who = 5) [ set x  1 set y  0 ]
end

to setup-4S ;;4 S Shape
  if (who = 1) [ set x  1 set y  0 ]
...
to setup-5T ;5 T-shaped
  if (who = 1) [ set x  2 set y  1 ]
  if (who = 2) [ set x  1 set y  0 ]
  if (who = 3) [ set x  2 set y  0]
  if (who = 4) [ set x  2 set y  -1 ]
  if (who = 5) [ set x  2 set y  -2]
end
