to go
  let x 1
  let list1 [2 3 4 5 1 2 1]
  let list2 [4 4 1 1 2 2 1]
  ;show filter [ [i] -> (equal i x) ] list1
  ;show numberOfxInl x list1
  show (myCount list1 list2 [])
  ; tick
end

to-report myCount [list1 list2 list3]
  (foreach list1
   [ [a] -> set list3 (sentence list3 (numberOfxinl a list2)) ])
  report list3
end

to-report numberOfxInl [x l]
  report length (myFilter x l)
end

to-report myFilter [x l]
 report filter [ [i] -> (equal i x) ] l
end
...
;Reports a list containing only those items of list for which the reporter reports true
;in other words, the items satisfying the given condition.
...
;length string
;Reports
;- the number of items in the given list, or
;- the number of characters in the given string.

;sentence value1 value2
;(sentence value1 ...)
;Makes a list out of the values.
;If any value is a list, its items are included in the result directly, rather than being included as a sublist. Examples make this clearer:
;show sentence 1 2
;=> [1 2]
;show sentence [1 2] 3
;=> [1 2 3]
...
;With a single list, runs the command for each item of list. command may be the name of a command, or an anonymous command created with -&;gt;.
;foreach [1.1 2.2 2.6] show
;=> 1.1
;=> 2.2
;=> 2.6
;foreach [1.1 2.2 2.6] [ [x] -> show (word x " -> " round i) ]
;=> 1.1 -> 1
;=> 2.2 -> 2
;=> 2.6 -> 3
